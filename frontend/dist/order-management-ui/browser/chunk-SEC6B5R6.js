...
var PostgrestFilterBuilder = class extends PostgrestTransformBuilder {
  ...
  */
  match(query2) {
    Object.entries(query2).forEach(([column, value]) => {
      this.url.searchParams.append(column, `eq.${value}`);
    });
    return this;
  }
...
var ItemService = class _ItemService {
  sb = inject(SupabaseService);
  getItems(page, size, search) {
    return from(this.sb.client.rpc("get_items_paged", {
      p_page: page,
      p_size: size,
      p_search: search || ""
    })).pipe(map(({ data, error }) => {
  ...
  getItem(id) {
    return from(this.sb.client.from("items").select("*").eq("id", id).eq("is_deleted", false).single()).pipe(map(({ data, error }) => {
      if (error)
        throw error;
  ...
  checkCode(code, excludeId) {
    return from((() => __async(this, null, function* () {
      let query2 = this.sb.client.from("items").select("id", { count: "exact", head: true }).eq("item_code", code).eq("is_deleted", false);
      if (excludeId) {
        query2 = query2.neq("id", excludeId);
      }
      const { count, error } = yield query2;
      if (error)
        throw error;
      return { success: true, data: (count ?? 0) === 0, message: "", errors: [] };
    }))());
  }
  createItem(request) {
    return from(this.sb.client.from("items").insert({
    ...
    }).select().single()).pipe(map(({ data, error }) => {
      if (error)
        throw error;
      return { success: true, data: this.mapRow(data), message: "", errors: [] };
    }));
  ...
  updateItem(id, request) {
    return from(this.sb.client.from("items").update(__spreadValues({
      item_code: request.itemCode,
      name_en: request.nameEN,
      name_ar: request.nameAR,
      category: request.category,
      price: request.price,
      vat_percentage: request.vatPercentage
    }, request.imagePath !== void 0 ? { image_path: request.imagePath } : {})).eq("id", id).select().single()).pipe(map(({ data, error }) => {
      if (error)
        throw error;
      return { success: true, data: this.mapRow(data), message: "", errors: [] };
    }));
  ...
  deleteItem(id) {
    return from(this.sb.client.from("items").update({ is_deleted: true }).eq("id", id)).pipe(map(() => void 0));
  }
  uploadImage(id, file) {
    const ext = file.name.split(".").pop() || "jpg";
    const path = `${id}.${ext}`;
    return from((() => __async(this, null, function* () {
      const { error: uploadError } = yield this.sb.client.storage.from(environment.storageBucket).upload(path, file, { upsert: true, contentType: file.type });
      if (uploadError)
        throw uploadError;
      const imageUrl = this.sb.getStorageUrl(path);
      yield this.sb.client.from("items").update({ image_path: imageUrl }).eq("id", id);
      return {
        success: true,
  ...
  parseCsvAndInsert(file) {
    return __async(this, null, function* () {
      ...
      const { error } = yield this.sb.client.from("items").insert(rows);
      if (error) {
  ...
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ItemService, factory: _ItemService.\u0275fac, providedIn: "root" });
};
...
//# sourceMappingURL=chunk-SEC6B5R6.js.map
